// admin.js — 管理页脚本（单/多 DU 自动识别 + DID 自动脚手架 + 合法/非法高亮 + 编辑/删除/导出）
// 说明：与 admin.html 中的布局/ID 一一对应；无需其它依赖。
...
(function () {
  ...
  const duHilite = el("du-hilite"); // 高亮层
  const tbl = el("tbl");
  ...
  // ====== 常量 / 状态 ======
  const DU_RE = /^DID\d{13}$/; // 合法格式：DID + 13 位数字
  ...
  // 把输入规范为：开头是 DID；任意 [空格/逗号/换行] 之后若没有 DID 则自动插入 DID；统一大写；清理零宽/BOM。
  function ensureDidScaffold(raw) {
  ...
  // 根据输入内容渲染高亮层（合法=绿，不合法=红；分隔符透明占位）
  function buildDuHighlightHTML(raw) {
    ...
    for (let i = 0; i < parts.length; i++) {
      ...
      out.push(
        `<span class="${ok ? "hl-ok" : "hl-bad"}">${escapeHtml(chunk)}</span>`
      );
    }
    // 尾部占位换行，帮助对齐高度
  ...
  // 解析 DU 输入（先做 DID 脚手架，再切分；去重）
  function parseDuInput() {
    const raw = (duInput.value || "").trim();
    if (!raw) return [];
    const scaffolded = ensureDidScaffold(raw);
    const list = scaffolded
      .split(/[\s,;]+/g)
      .map((s) => s.trim())
      .filter(Boolean);
  ...
  function renderRows(items) {
    tbody.innerHTML = items
      .map((it) => {
        ...
        const link = it.photo_url
          ? `<a href="${toAbsUrl(it.photo_url)}" target="_blank">查看</a>`
          : "";
        const remark = it.remark ? String(it.remark).replace(/[<>]/g, "") : "";
        const act = `
        <div class="actions">
          <button class="btn" data-act="edit" data-id="${it.id}" data-du="${it.du_id}" data-status="${
          it.status || ""
        }" data-remark="${escapeHtml(remark)}">编辑</button>
  ...
  async function onDelete(id) {
    ...
    try {
      const resp = await fetch(`${API_BASE}/api/du/update/${id}`, {
        method: "DELETE",
      });
      const text = await resp.text();
      let data = null;
  ...
  async function exportAll() {
    try {
      ...
      const firstUrl = `${API_BASE}${
        q.mode === "batch" ? "/api/du/batch?" : "/api/du/search?"
      }${p1.toString()}`;
      ...
      if (!fResp.ok) {
        throw new Error(
          (fData && (fData.detail || fData.message)) || "HTTP " + fResp.status
        );
      }
  ...
  el("btn-reset").onclick = () => {
    ["f-status", "f-remark", "f-has", "f-from", "f-to", "f-ps2"].forEach(
      (id) => {
        const n = el(id);
        n.value = id === "f-ps2" ? "20" : "";
      }
    );
    duInput.value = "DID"; // 重置后默认显示 DID
  ...
  duInput.addEventListener("input", () => {
    ...
    const after = ensureDidScaffold(before);
    if (before !== after) {
      duInput.value = after;
      try {
  ...
  window.addEventListener("load", () => {
    if (!duInput.value.trim()) duInput.value = "DID";
    duHilite.innerHTML = buildDuHighlightHTML(duInput.value);
    // 初始不自动查询，保持空态文案
