...
export function setupDnAdminPage(rootEl, { i18n, applyTranslations } = {}) {
  ...
  const refreshSheetBtn = el('btn-refresh-sheet');
  ...
  async function triggerSheetRefresh() {
    if (!refreshSheetBtn) return;
    const loadingText = translate(
      'actions.refreshing',
      '正在触发后台拉取 Google 表格…'
    );
    if (hint) {
      hint.textContent = loadingText;
    }
  ...
  refreshSheetBtn?.addEventListener(
    'click',
    () => {
      if (refreshSheetBtn?.disabled) return;
      triggerSheetRefresh();
    },
    { signal }
  );
