...
const STATUS_LABEL_KEYS = {
  ...
  'WAITING PIC FEEDBACK': 'status.waitingPic',
  'REPLAN MOS DUE TO LSP DELAY': 'status.replanLsp',
...
export function setupDashboardPage(rootEl, options = {}) {
  if (!rootEl) {
    return {
      destroy() {},
      refreshTranslations() {},
  ...
  const { i18n, applyTranslations } = options || {};
  ...
  const getOverallTotal = () => {
    ...
    const sums = computeTotals(RAW_ROWS);
    return sums[sums.length - 1] ?? null;
  };

  const updateMetaSummary = (date, total) => {
    const meta = $('#meta');
    if (!meta) return;
    if (date) {
      const totalValue = total ?? getOverallTotal();
      meta.textContent = t('header.meta', {
        date,
        total: totalValue ?? '—',
      });
    } else {
      meta.textContent = t('header.metaPlaceholder');
    }
  };
  ...
  const updateToggleZeroButton = () => {
    ...
    btn.textContent = mutedZero ? t('actions.highlightNonZero') : t('actions.showAll');
    btn.setAttribute('title', t('actions.toggleZeroTitle'));
  };
  ...
  function render() {
    ...
    updateMetaSummary(RAW_ROWS[0]?.date || '', totals[totals.length - 1]);
    ...
    if (thead) {
      ...
      thGroup.textContent = t('table.group');
  ...
  $('#dl')?.addEventListener(
    ...
    () => {
      const rows = currentRows();
      const header = [t('csv.groupHeader'), ...STATUS_ORDER.map((s) => statusLabel(s))].join(',');
